...
export function Step2TipoImovel({ onNext }: { onNext: () => void }) {
  ...
  const [selected, setSelected] = useState<TipoImovel | "">(
    financiamentoImobiliario.tipo_imovel as TipoImovel | ""
  )

  function handleNext() {
    if (!selected) return
    setFinanciamentoImobiliario({ tipo_imovel: selected })
    pushDataLayer("step_completed", { funil: "financiamento_imobiliario", step: 2, tipo_imovel: selected })
    onNext()
  }
    ...
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Selecione a categoria que melhor descreve o imóvel a ser financiado.
        </p>
      </div>
