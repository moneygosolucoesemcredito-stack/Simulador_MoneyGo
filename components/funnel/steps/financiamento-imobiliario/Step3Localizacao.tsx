...
export function Step3Localizacao({ onNext, onNaoQualificado }: { onNext: () => void; onNaoQualificado: () => void }) {
  const { financiamentoImobiliario, setFinanciamentoImobiliario } = useFunnelStore()
  const [cep, setCep] = useState(financiamentoImobiliario.cep || "")
  const [logradouro, setLogradouro] = useState(financiamentoImobiliario.logradouro || "")
  const [bairro, setBairro] = useState(financiamentoImobiliario.bairro || "")
  const [cidade, setCidade] = useState(financiamentoImobiliario.cidade || "")
  const [uf, setUf] = useState(financiamentoImobiliario.uf || "")
  ...
  function handleNext() {
    if (!cidade || !uf) return
    setFinanciamentoImobiliario({ cep, logradouro, bairro, cidade, uf })
    pushDataLayer("step_completed", { funil: "financiamento_imobiliario", step: 3, cidade, uf })
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <h2 className="text-2xl font-bold tracking-tight">Onde fica o imóvel?</h2>
        <p className="text-muted-foreground text-sm">
          Informe o CEP do imóvel a ser financiado.
        </p>
      </div>
