...
export function Step6Resultado({ onNext }: { onNext: () => void }) {
  const { homeEquity } = useFunnelStore()
  const { taxaMensal } = CONFIG.homeEquity

  const resultado = useMemo(
    () => calcularSimulacao(homeEquity.valor_solicitado, taxaMensal, homeEquity.prazo_meses),
    [homeEquity.valor_solicitado, homeEquity.prazo_meses, taxaMensal]
  )

  function handleNext() {
    pushDataLayer("step_completed", { funil: "home_equity", step: 6 })
    onNext()
  }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Veja as condições estimadas para o seu crédito.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--gold)]/30 bg-[var(--gold)]/5 p-6 space-y-5">
        <div className="flex justify-between items-start">
          ...
          <span className="font-semibold">{formatarMoeda(homeEquity.valor_solicitado)}</span>
        </div>
        <div className="flex justify-between items-start">
          <span className="text-sm text-muted-foreground">Prazo</span>
          <span className="font-semibold">{homeEquity.prazo_meses} meses</span>
        </div>
      ...
      <div className="flex gap-2 rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
        ...
        <p>
          Taxa sujeita a análise de crédito. Modalidade pós-fixada com correção monetária por
          indexador (IPCA/IGP-M). Valores estimados, sem compromisso.
        </p>
      </div>
